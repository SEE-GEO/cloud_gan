...
class CloudSat_Modis_Aux(CloudSatBase):
    ...
    @property
    def modis_uncertainty_index(self):
        raw_data = (self["EV_1KM_Emissive_Uncert_Indexes"][:])
        mask = raw_data == 255
        data = np.array(raw_data, dtype=np.float32)
        return ma.masked_array(data,mask=mask)

    @property
    def modis_specified_uncertainty(self):
        raw_data = (self["EV_1KM_Emissive_spec_uncert"][:])
        mask = raw_data == -999
        data = np.array(raw_data, dtype=np.float32)
        return ma.masked_array(data, mask=mask)

    @property
    def modis_uncertainty_scaling_factor(self):
        raw_data = (self["EV_1KM_Emissive_scaling_factor"][:])
        mask = raw_data == -999
        data = np.array(raw_data, dtype=np.float32)
        return ma.masked_array(data, mask=mask)


class CloudSat_ECMWF_Aux(CloudSatBase):
    pattern = re.compile("([\d]*)_([\d]*)_CS_ECMWF-AUX_GRANULE_P_R([\d]*)_E([\d]*)\.*")
    ...
    @property
    def ecmwf_temperature(self):
        raw_data = (self["Temperature"][:])
        mask = raw_data == -999
        data = np.array(raw_data, dtype=np.float32)
        return ma.masked_array(data, mask=mask)
    @property
    def EC_height(self):
        raw_data = (self["EC_height"][:])
        mask = raw_data == -9999
        data = np.array(raw_data,dtype=np.float32)
        return ma.masked_array(data,mask=mask)