...
plt.savefig('auto_corr_gan_average' + str(epoch))
#f.close()
...
ax.tick_params(axis='both', which='major', labelsize=26)
ax.set_aspect(aspect)
pcm1 = ax.pcolormesh(xplot,yplot,average_auto_corr_iaaft,vmin=-1,vmax=1, cmap='seismic')
cbar1 = f.colorbar(pcm1, orientation = 'horizontal', fraction =0.049, pad=0.15)
...
cbar1.ax.tick_params(labelsize=26, rotation=45)
plt.savefig('auto_corr_cs_gan_difference_average' + str(epoch))
