...
for ind in range(0,80,20):
    ...
    for k in range(0,8):
        print('average modis channel ',k,torch.mean(modis_scenes[:,:,:,k]))
    ...
    if path.exists(folder + 'network_parameters_CGAN.pt'):
        ...
        with torch.no_grad():
            ...
            checkpoint = torch.load(folder + 'network_parameters_CGAN.pt', map_location=torch.device('cpu'))
            netG.load_state_dict(checkpoint['model_state_dict_gen'])
            netG.zero_grad()
            print('dadada')
            all_generated = []

            generated_scenes = netG(torch.randn(D_gen), modis_scenes)
