...
for i in range(0,64):
    iaaft_histogram, bin_edges_iaaft = np.histogram(iaaft_scenes[:, i], bins=num_bins, range=(-34, 20), density=True)
    cloudsat_histogram, bin_edges_cloudsat = np.histogram(cloudsat[:, i], bins=num_bins, range=(-34, 20), density=True)
    total_iaaft_histogram[i] = iaaft_histogram
    total_cloudsat_histogram[i] = cloudsat_histogram
...
pcm1 = ax1.pcolormesh(x_hist, y_hist, new_total_iaaft)
...
cb1.set_label('Norm. occurrence (IAAFT)', fontsize=28)
...
pcm2 = ax2.pcolormesh(x_hist, y_hist, new_total_cloudsat)
...
cb2.set_label('Norm. occurrence (Real)', fontsize=28)
...
pcm3 = ax3.pcolormesh(x_diff, y_diff, occurence_diff, vmin = -0.003, vmax = 0.003, cmap= 'seismic')
...
cb3.set_label('Occurence bias (Real - IAAFT)', fontsize=28)
