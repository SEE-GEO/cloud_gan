...
def Create_Histogram(epoch):
    ...
    if path.exists(folder + 'network_parameters_' + str(epoch) + '.pt'):

        with torch.no_grad():

            checkpoint = torch.load(folder + 'network_parameters_' + str(epoch) + '.pt', map_location=torch.device('cpu'))
        ...
        name_string = 'generated_scenes_for_histogram_' + str(epoch)
    ...
    location = './'
    file_string = location + 'GAN_generated_scenes_for_histogram_'+ str(epoch) + '.h5'
    hf = h5py.File(file_string, 'r')
    ...
    #f2.tight_layout()
    plt.savefig('Histogram_GAN_generated' + str(epoch))
    #plt.show()
    ...
    #f3.tight_layout()
    print('Percentage_histogram_GAN_real_and_generated' + str(epoch))
    plt.savefig('Difference_histogram_GAN_real_and_generated' + str(epoch))
    ...
    #f3.tight_layout()
    print('Percentage_histogram_GAN_real_and_generated' + str(epoch))
    plt.savefig('Percentage_histogram_GAN_real_and_generated' + str(epoch))
