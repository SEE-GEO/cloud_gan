...
class GAN_discriminator (torch.nn.Module):
    def __init__(self, D_in, H, D_out):
        # H=[256, 128, 128, 5, 6, 64, 128, 256, 256, 4096, 1]
        super(GAN_discriminator, self).__init__()
        #region
        self.upsample0 = torch.nn.ConvTranspose2d(D_in[1],D_in[1],(4,1), stride=(4,1))
        self.convolution0 = torch.nn.Conv2d(D_in[1],H[0],(5,3),padding=(2,1))
        #relu
        self.batchNorm0 = torch.nn.BatchNorm2d(H[0])

        self.upsample1 = torch.nn.ConvTranspose2d(H[0], H[0], (4, 1), stride=(4, 1))
        self.convolution1 = torch.nn.Conv2d(H[0], H[1], (5, 3), padding=(2, 1))
        # relu
        self.batchNorm1 = torch.nn.BatchNorm2d(H[1])

        self.upsample2 = torch.nn.ConvTranspose2d(H[1], H[1], (2, 1), stride=(2, 1))
        self.convolution2 = torch.nn.Conv2d(H[1], H[2], (3, 3), padding=(1, 1))
        # relu
        self.batchNorm2 = torch.nn.BatchNorm2d(H[2])

        self.upsample3 = torch.nn.ConvTranspose2d(H[2], H[2], (2, 1), stride=(2, 1))
        self.convolution3 = torch.nn.Conv2d(H[2], H[3], (3, 3), padding=(1, 1))
        # relu
        self.batchNorm3 = torch.nn.BatchNorm2d(H[3])
        #endregion

        #concatenate

        self.convolution5 = torch.nn.Conv2d(H[4],H[5],(3,3),stride=(2,2),padding=(1,1))
        #relu

        self.convolution6 = torch.nn.Conv2d(H[5], H[6], (3, 3), stride=(2, 2),padding=(1,1))
        # relu

        self.convolution7 = torch.nn.Conv2d(H[6], H[7], (3, 3), stride=(2, 2),padding=(1,1))
        # relu

        self.convolution8 = torch.nn.Conv2d(H[7], H[8], (3, 3), stride=(2, 2),padding=(1,1))
        # relu

        #flatten
        self.dense9=torch.nn.Linear(H[9],H[10])
