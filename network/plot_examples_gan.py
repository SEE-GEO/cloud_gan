...
for k in range(0, 4):
    ...
    f, axs = plt.subplots(4, 4, figsize=(9.6, 8))
    file_number = 0
    counter = 0
    ...
    for i in range(0, 4):
        ...
        for j in range(0, 4):
            ...
            pcm = axs[i, j].pcolormesh(xplot, yplot, np.transpose(output.detach().numpy()[i + 4 * j][0]), norm=norm)
    ...
    cbar_ax = f.add_axes([0.9, 0.08, 0.025, 0.9]) #([distance from left, bottom height, width, height])
    ...
    plt.savefig('./Results/testepoch' + str(epoch) + '_GAN_ver4_final' + str(k))